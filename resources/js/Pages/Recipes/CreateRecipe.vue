...
const form = useForm({
    ...
    ingredients: attrs.recipe
        ? attrs.recipe.data.ingredients.map((i) => {
              return {
                  id: null,
        ...
        : [],
    categories:
        attrs.recipe && attrs.recipe.data.categories
            ? attrs.recipe.data.categories.map((c) => c.slug)
            : [],
    image: null,
...
onMounted(() => {
    ...
    if (existingRecipe) {
        form.ingredients = attrs.recipe.data.ingredients.map((i) => {
            return {
                id: i.id,
                ingredient: `${i.quantity} ${i.unit} ${i.name}${
                    i.notes ? ", " + i.notes : ""
                }`,
                parse: false,
            };
        });
        form.imageUrl = attrs.recipe.data.images
            ? "/storage/" + attrs.recipe.data.images[0].filename
            : placeholderImage;
    }
});
    ...
    <BreezeAuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{
                    $attrs.recipe && $attrs.recipe.data.id
                        ? "Update your recipe"
                        : "Create a recipe"
                }}
            </h2>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        >
                            <div
                                class="md:flex flex-row-reverse justify-between align-middle"
                            >
                                <!-- Image -->
                                <div
                                    class="overflow-hidden relative m-auto md:m-0"
                                >
                                    <div
                            ...
                            <div>
                                ...
                                >
                                    <input
                                        ...
                                        v-model="
                                            form.ingredients[index].ingredient
                                        "
                                    ...
                                    >
                                        <p class="absolute right-3 bottom-1.5">
                                            x
                                        </p>
                                    </div>
                                </div>
                                ...
                                >
                                    <p class="absolute right-3 bottom-1.5">+</p>
                                </div>
                            </div>
